...
</style></head><body><main class="wrap"><div class="eyebrow">LOCAL ARCHIVE</div><h1>抖音公开作品归档</h1><p>粘贴博主主页或抖音分享短链，设置好起始位置和数量后点击开始。首次使用时，请在弹出的浏览器窗口中自行登录或处理验证码。</p><section class="card"><div class="row"><input id="url" placeholder="粘贴 https://v.douyin.com/... 或博主主页链接"><button id="start">开始归档</button></div><div class="options"><label>从第 <input id="from" type="number" min="1" value="1"> 个作品开始</label><label>本批下载 <input id="max" type="number" min="1" max="500" value="20"> 个</label><span class="tip">已下载过的作品会自动跳过；博主作品抓完后程序会自动停止。一批次只打开一次浏览器。</span></div><div class="status"><div class="state" id="state">等待开始</div><div class="log" id="log">尚未执行任务。</div></div><p class="hint">文件保存位置：<code id="folder"></code>（每个博主一个文件夹，视频以「发布日期_标题_作品ID」直接命名）</p></section></main><script>
const $=id=>document.getElementById(id);const labels={idle:'等待开始',running:'正在归档，请勿关闭浏览器',done:'任务完成',failed:'任务失败'};async function refresh(){const r=await fetch('/api/status');const s=await r.json();$('state').textContent=labels[s.status]||s.status;$('log').textContent=s.lines.join('\\n')||'尚未执行任务。';$('folder').textContent=s.outputDir;$('start').disabled=s.status==='running';$('start').textContent=s.status==='running'?'正在运行…':'开始归档';$('log').scrollTop=$('log').scrollHeight}$('start').onclick=async()=>{const r=await fetch('/api/archive',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url:$('url').value.trim(),max:Number($('max').value),start:Number($('from').value)})});const d=await r.json();if(!r.ok)alert(d.error);refresh()};refresh();setInterval(refresh,1200);
</script></body></html>`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/archive') {
    ...
    activeChild.on('error', error => { addLine(error.message); job.status = 'failed'; activeChild = null; });
    activeChild.on('close', code => { job.status = code === 0 ? 'done' : 'failed'; addLine(code === 0 ? '归档完成。' : `任务结束，退出码 ${code}。`); activeChild = null; });
